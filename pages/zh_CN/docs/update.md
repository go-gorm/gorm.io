...
对于更新操作，GORM 支持 `BeforeSave`、`BeforeUpdate`、`AfterSave`、`AfterUpdate` Hook，这些方法将在更新记录时被调用，查看 [Hook](hooks.html) 获取详情
...
若要在 Before Hook 中改变要更新的值，如果它是一个完整的更新，可以使用 `Save`；否则，应该使用 `scope.SetColumn` ，例如：
