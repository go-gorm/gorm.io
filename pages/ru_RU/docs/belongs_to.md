...
```go
// `User` относится один к одному к `Company`, а `CompanyID` - это внешний ключ
type User struct {
  gorm.Model
  Name      string
  CompanyID int
...
Чтобы определить отношение "один-к-одному", внешний ключ должен существовать, внешний ключ по умолчанию использует имя типа владельца плюс имя его первичного поля.
...
Для отношения "один-к-одному" GORM обычно использует первичное поле владельца в качестве значения внешнего ключа, в приведенном выше примере это поле `Company` `ID`.
